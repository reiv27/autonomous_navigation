...
def get_edge_cost(parent, child, occ_map):
  ...
  # if either cell is a hard obstacle -> infinite cost
  # obstacle_threshold = 0.7
  # if occ_map[px, py] > obstacle_threshold or occ_map[cx, cy] > obstacle_threshold:
  #   return np.inf

  # base movement cost: euclidean (1 for 4-neigh, sqrt(2) for diagonals)
  base = np.linalg.norm(np.array([cx - px, cy - py]).astype(float))

  # soft penalty for occupancy (prefer low-occupancy cells)
  # average occupancy of the two cells
  avg_occ = 0.5 * (occ_map[px, py] + occ_map[cx, cy])
  softness = 5.0  # small-ish weight; can be tuned
  edge_cost = base * (1.0 + softness * avg_occ)
...
def run_path_planning(occ_map, start, goal):
  ...
  while not np.array_equal(parent, goal):
    ...
    for (nx, ny) in get_neighborhood(parent, occ_map.shape):
      if closed_flags[nx, ny] == 1:
        continue  # already processed

      edge = get_edge_cost(parent, (nx, ny), occ_map)
      if np.isinf(edge):
        continue  # impassable

      tentative = costs[parent[0], parent[1]] + edge
  ...
  if np.array_equal(parent, goal):
    ...
    print("found goal     :", parent) 
    print("cells expanded :", np.count_nonzero(closed_flags))
